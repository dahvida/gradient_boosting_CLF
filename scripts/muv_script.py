"""
Script to get the results for the MUV dataset
"""
import sys
sys.path.append('..')
...
fc_box = eval_loss_molnet("Focal_loss", dataset, task_n, average_precision_score)
fc_box = np.mean(fc_box, axis=1)

la_box = eval_loss_molnet("LA_loss", dataset, task_n, average_precision_score)
la_box = np.mean(la_box, axis=1)

eq_box = eval_loss_molnet("EQ_loss", dataset, task_n, average_precision_score)
eq_box = np.mean(eq_box, axis=1)

ldam_box = eval_loss_molnet("LDAM_loss", dataset, task_n, average_precision_score)
ldam_box = np.mean(ldam_box, axis=1)

#run benchmarks on baseline
loss = None
wce_box = np.empty((50, 17, 8))   
for j in range(17):
